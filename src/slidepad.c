...
errno_t wanbeiyu_slidepad_hold(WanbeiyuSlidepad *sp, uint8_t x, uint8_t y)
{
    ...
    errno_t horizontal_err = WANBEIYU_SLIDEPAD_AXIS_NEUTRAL < x
                                 ? /* 129, 130, ..., 255 */ wanbeiyu_idac_set_sink(sp->horizontal, (uint8_t)wanbeiyu_internal_remap(x, 129, UINT8_MAX, 0, UINT8_MAX))
                                 : /* 128, 127, ..., 0 */ wanbeiyu_idac_set_source(sp->horizontal, (uint8_t)wanbeiyu_internal_remap(x, 128, 0, 0, UINT8_MAX));
    errno_t vertical_err = WANBEIYU_SLIDEPAD_AXIS_NEUTRAL < y
                               ? wanbeiyu_idac_set_sink(sp->vertical, (uint8_t)wanbeiyu_internal_remap(y, 129, UINT8_MAX, 0, UINT8_MAX))
                               : wanbeiyu_idac_set_source(sp->vertical, (uint8_t)wanbeiyu_internal_remap(y, 128, 0, 0, UINT8_MAX));
    if (horizontal_err != 0 ||
        vertical_err != 0)
    {
        return EIO;
